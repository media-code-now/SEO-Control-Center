...
export async function GET(request: NextRequest) {
  ...
  try {
    await importGa4ForIntegration(integration.id);
  } catch (importError) {
    console.error("Initial GA4 import failed", importError);
  }

  return NextResponse.redirect(`${env.NEXTAUTH_URL}/settings?connected=ga4`);
}
